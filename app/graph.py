...
def _build_selection_reasoning(
    ...
    if selected.get(
        ...
        selected_reasons.append(
            "It satisfies every automatically "
        ...
        selected_reasons.append(
            f"It has {selected_violation_count} "
            "policy or traveller-constraint "
            "violation(s), which is the lowest "
            "available violation count."
        )

    if not selected_inventory_review:
        selected_reasons.append(
            "Its required live-inventory "
            "fields were available for "
    ...
    if selected_arrival_time:
        selected_reasons.append(
            ...
            f"arrives at {selected_arrival_time}."
        )
    ...
    if same_priority_options:
        ...
        if (
            selected_total
            <= lowest_same_priority_cost
        ):
            selected_reasons.append(
                "It has the lowest total cost "
                "among options with the same "
                "policy and verification priority."
            )

    if policy_manual_review_required:
        selected_reasons.append(
            "The uploaded policy still contains "
            "clauses that require human review, "
            "but those clauses were not silently "
            "converted into automatic rules."
        )

    selected_flight_id = str(
        selected_flight.get("id")
        or selected_flight.get(
            "flight_number"
        )
        or ""
    )
    ...
    for alternative in (
        ...
        if (
            ...
            rejection_summary = (
                "Rejected because important "
                "live-inventory fields require "
                "manual verification."
            )

        else:
            rejection_summary = (
                "Ranked lower after TripGuard "
                "applied its policy-first "
                "decision order."
            )
    ...
    return {
        "strategy":
            (
                "TripGuard ranks options by "
                "policy and traveller compliance "
                "first, unresolved live-data "
                "verification second, total cost "
                "third and arrival time fourth."
            ),
        "priority_order":
            [
                (
                    "Fewest policy and "
                    "traveller violations"
                ),
                (
                    "No unresolved "
...
def evaluate_options_node(
    ...
    for flight in flights:
        for hotel in hotels:
            ...
            if required_class:
                if not flight_class:
                    warnings.append(
                        "Flight class could not "
                        "be verified from live "
                        "inventory."
                    )
                    ...
                    violations.append(
                        "Domestic flight is not "
                        "in the travel class "
                        "permitted by the active "
                        "policy."
                    )
            ...
            if (
                ...
                if hotel_distance is None:
                    warnings.append(
                        "Hotel distance from the "
                        "work location could not "
                        "be verified automatically."
                    )

                    manual_review_reasons.append(
                        "Verify the selected "
                        "hotel's distance from "
                        "the work location."
                    )
                    ...
                    violations.append(
                        "Hotel is farther from the "
                        "work location than the "
                        "active policy permits."
                    )
            ...
            if arrival_before:
                if not arrival_time:
                    warnings.append(
                        "Flight arrival time could "
                        "not be verified from live "
                        "inventory."
                    )
            ...
            if (
                ...
                violations.append(
                    "Trip cost exceeds the "
                    "traveller's stated budget."
                )
            ...
            if (
                ...
                warnings.append(
                    ...
                    "active policy's cost threshold."
                )
    ...
    if (
        ...
        message += (
            ...
            "option(s) contain inventory "
            "fields requiring manual "
            "verification."
        )
...
def select_recommendation_node(
    ...
    if compliant_options:
        ...
        explanation = (
            ...
            "the option with the fewest "
            "policy violations, then preferred "
            "verified inventory and the lowest "
            "total cost."
        )
